...
public class SubRevenueController {
   ...
   private SubRevenueSourceServices service;
    ...
    @GetMapping("/SubRevenueSourceById/{id}")
    public ResponseEntity<Response<SubRevenueResource>> subRevenueSourceById(@PathVariable Long id){

        logger.info("Received request sub revenue source: {}", id);
        Response<SubRevenueResource> response = new Response<>();

        try {

            Optional<SubRevenueResource> createdSub = service.subRevenueSourceById(id);
    ...
    @GetMapping("/SubRevenueSourceAll")
    public ResponseEntity<Response<SubRevenueResource>> subRevenueSourceAll(){

        logger.info("Received request sub revenue source: {}");
        Response<SubRevenueResource> response = new Response<>();

        try {

            List<SubRevenueResource> getSubRevenueResource = service.subRevenueResourceAll();
            if (getSubRevenueResource != null) {

                response.setCode("200");
                response.setMessage("Revenue Source Created Successfully");
                response.setData(getSubRevenueResource);
                return new ResponseEntity<>(response, HttpStatus.OK);
            } else {
